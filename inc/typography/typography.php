...
if ( ! function_exists( 'triphub_typography_default_fonts' ) ) :
/**
 * Set the default system fonts.
 */
function triphub_typography_default_fonts() {
	$fonts = array(
		'Default Family',
		'System Stack',
	...
	return apply_filters( 'triphub_typography_default_fonts', $fonts );
}
endif;

if( ! function_exists( 'triphub_get_font_family' ) ) :
/**
 * Get font family 
 */
function triphub_get_font_family( $font ){
	$output = '';
	$no_quotes = array(
		'inherit',
		'Arial, Helvetica, sans-serif',
	...
	if( $font['family'] === 'System Stack' ){
		$output = apply_filters( 'triphub_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
	}else{
...
if ( ! function_exists( 'triphub_google_fonts_url' ) ) :	
/**
 * Google Fonts url
 */
function triphub_google_fonts_url() {
	$defaults  			= triphub_get_typography_defaults();
	$generalDefaults    = triphub_get_general_defaults();
	...
	$not_google = str_replace( ' ', '+', triphub_typography_default_fonts() );
	...
	$google_fonts = apply_filters( 'triphub_typography_google_fonts', $google_fonts );

	$subset = apply_filters( 'triphub_fonts_subset', '' );
	...
	$display = apply_filters( 'triphub_google_font_display', 'swap' );
	...
	if( get_theme_mod( 'ed_localgoogle_fonts', $generalDefaults['ed_localgoogle_fonts'] ) ) {
        $fonts_url = triphub_get_webfont_url( add_query_arg( $font_args, 'https://fonts.googleapis.com/css' ) );
    }
...
if ( ! function_exists( 'triphub_get_all_google_fonts_ajax' ) ) :
/**
 * Return an array of all of our Google Fonts.
 */
function triphub_get_all_google_fonts_ajax() {
	// Bail if the nonce doesn't check out
	if ( ! isset( $_POST['triphub_customize_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['triphub_customize_nonce'] ), 'triphub_customize_nonce' ) ) {
		wp_die();
	}

	// Do another nonce check
	check_ajax_referer( 'triphub_customize_nonce', 'triphub_customize_nonce' );
	...
	$fonts = triphub_get_all_google_fonts();
...
add_action( 'wp_ajax_triphub_get_all_google_fonts_ajax', 'triphub_get_all_google_fonts_ajax' );

if( ! function_exists( 'triphub_flush_local_google_fonts' ) ){
	/**
	 * Ajax Callback for flushing the local font
	 */
  	function triphub_flush_local_google_fonts() {
		$WebFontLoader = new triphub_WebFont_Loader();
		//deleting the fonts folder using ajax
		$WebFontLoader->delete_fonts_folder();
		die();
  	}
}
add_action( 'wp_ajax_flush_local_google_fonts', 'triphub_flush_local_google_fonts' );
add_action( 'wp_ajax_nopriv_flush_local_google_fonts', 'triphub_flush_local_google_fonts' );