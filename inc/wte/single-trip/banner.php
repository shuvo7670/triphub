<?php
/**
 * TripHub Single Trip Banner functions lists here
 *
 * @package triphub
 */
...
if( ! function_exists( 'triphub_single_trip_feature_image' ) ) :
/**
 * Single Trip Feature Image
 *  
 * @package triphub
*/
function triphub_single_trip_feature_image(){ 
    global $post;
	...
	$wp_travel_engine_options = get_option('wp_travel_engine_settings');
    $displayFeatImage         = $wp_travel_engine_options['show_featured_image_in_gallery'];
    ...
    <div class="tmp-gallery<?php echo esc_attr( $trip_class); ?>">
        <div class="<?php echo esc_attr( $owl_class ); ?>">
            <?php 
            /**
             * @hooked triphub_content_start - 10
            */
            if( $displayFeatImage === 'yes') do_action( 'triphub_before_trip_entry_content' );
...
if( ! function_exists( 'triphub_wte_gallery_override' ) ) :
/**
 ...
 * @package triphub
*/
function triphub_wte_gallery_override(){
    global $post;
	$social_icons              = new triphub_Social_Lists;
	$wpte_trip_images          = get_post_meta( $post->ID, 'wpte_gallery_id', true );
	$wp_travel_engine_settings = get_option('wp_travel_engine_settings');
	$displayFeatImage          = $wp_travel_engine_settings['show_featured_image_in_gallery'];
        ...
        if( count( $wpte_trip_images ) > 1 ){
            ...
            <div class="st-gallery-wrapper">
                <button class="gallery-btn">
                    <?php echo wp_kses( $social_icons->triphub_lists_all_svgs( 'image' ), triphub_get_kses_extended_ruleset() ); ?>
                    <span><?php esc_html_e( 'Gallery', 'triphub' ); ?></span>                    
                </button>
                <script type="text/javascript">
                    jQuery(document).ready(function($){ 
