 ...
 * @return void
 */
function triphub_customize_partial_blogname() {
	bloginfo( 'name' );
}
endif;

if ( ! function_exists( 'triphub_customize_partial_blogdescription' ) ) :
/**
 * Render the site tagline for the selective refresh partial.
 *
 * @return void
 */
function triphub_customize_partial_blogdescription() {
	bloginfo( 'description' );
}
endif;

if( ! function_exists( 'triphub_header_phone_label' ) ) :
/**
 * Header Email
*/
function triphub_header_phone_label(){
    $defaults    = triphub_get_general_defaults();
    $phone_label = get_theme_mod( 'triphub_phone_label', $defaults['triphub_phone_label'] );
    if( $phone_label ) echo '<div class="contact-phone-label">' . esc_html( $phone_label ) . '</div>';
}
endif;

if( ! function_exists( 'triphub_header_phone' ) ) :
/**
 * Header Phone
*/
function triphub_header_phone(){    
    if( triphub_has_contact_image() ){
        $class = 'head-5-dtls';
    }else{
        $class = 'tel-link';
    }
    ...
    if( $phone ){ ?>
        <div class="contact-phone-wrap">
            <?php if( triphub_has_contact_image() ){ ?>
...
if( ! function_exists( 'triphub_header_email_label' ) ) :
/**
 * Header Email Label
*/
function triphub_header_email_label(){
    $defaults    = triphub_get_general_defaults();
    $email_label = get_theme_mod( 'triphub_email_label', $defaults['triphub_email_label'] );
    if( $email_label ) echo '<div class="contact-email-label">' . esc_html( $email_label ) . '</div>';
}
endif;

if( ! function_exists( 'triphub_header_email' ) ) :
/**
 * Header Email
*/
function triphub_header_email(){
    $defaults = triphub_get_general_defaults();
...
if( ! function_exists( 'triphub_header_mobile_menu_label' ) ) :
/**
 * Menu Label
*/
function triphub_header_mobile_menu_label(){
    $defaults   = triphub_get_general_defaults();
...
if ( ! function_exists( 'triphub_get_header_button' ) ) :
    /**
     * Header Button Text
     */
    function triphub_get_header_button() {
        $defaults = triphub_get_general_defaults();
...
if ( ! function_exists( 'triphub_get_home_text' ) ) :
/**
 * Breadcrumb Home Text
 */
function triphub_get_home_text() {
    $defaults = triphub_get_general_defaults();
    return esc_html( get_theme_mod( 'home_text', $defaults['home_text'] ) );
}
endif;

if ( ! function_exists( 'triphub_get_single_related_title' ) ) :
    /**
     * Single Related Title
     */
    function triphub_get_single_related_title() {
        $defaults = triphub_get_general_defaults();
...
if( ! function_exists( 'triphub_get_footer_copyright' ) ) :
/**
 * Footer Copyright
*/
function triphub_get_footer_copyright(){
    $defaults  = triphub_get_general_defaults();
    $copyright = get_theme_mod( 'footer_copyright', $defaults['footer_copyright'] );
    echo '<span class="copyright">';
    if( $copyright ){
        if( triphub_pro_is_activated() ){
            echo wp_kses_post( triphub_apply_theme_shortcode( $copyright ) );
        } else {
            echo wp_kses_post( $copyright);
        }
    }else{
        esc_html_e( '&copy; Copyright ', 'triphub' );
        echo date_i18n( esc_html__( 'Y', 'triphub' ) );
...
if( ! function_exists( 'triphub_payment_label' ) ) :
/**
 * Header Email
*/
function triphub_payment_label(){
    $defaults      = triphub_get_general_defaults();
...
if( ! function_exists( 'triphub_get_blog_read_more' ) ) :
/**
 * Blog Read More Button
*/
function triphub_get_blog_read_more(){
    $defaults      = triphub_get_general_defaults();
    return esc_html( get_theme_mod( 'blog_read_more',  $defaults['blog_read_more'] ) );
}
endif;

if( ! function_exists( 'triphub_get_ed_blog_title' ) ) :
/**
 * Blog Page Title
*/
function triphub_get_ed_blog_title(){
    $defaults      = triphub_get_general_defaults();
...
if( ! function_exists( 'triphub_get_ed_blog_desc' ) ) :
/**
 * Blog Page Description
*/
function triphub_get_ed_blog_desc(){
    $defaults     = triphub_get_general_defaults();
...
if( ! function_exists( 'triphub_get_ed_archive_title' ) ) :
/**
 * Archive Page Title
*/
function triphub_get_ed_archive_title(){
    $defaults         = triphub_get_general_defaults();
...
if( ! function_exists( 'triphub_get_ed_archive_description' ) ) :
/**
 * Archive Page Description
*/
function triphub_get_ed_archive_description(){
    $defaults               = triphub_get_general_defaults();
...
if( ! function_exists( 'triphub_get_ed_archive_post_count' ) ) :
/**
 * Archive Page Post Count
*/
function triphub_get_ed_archive_post_count(){
    $defaults               = triphub_get_general_defaults();
...
if ( ! function_exists( 'triphub_get_social_share_text' ) ) :
/**
 * Social Share Text
 */
function triphub_get_social_share_text() {
    $defaults = triphub_get_general_defaults();
