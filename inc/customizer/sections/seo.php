...
function triphub_customize_register_seo_setting( $wp_customize ){
    ...
    $wp_customize->add_control(
		new triphub_Toggle_Control( 
			...
			array(
				...
				'label'	      => __( 'Show Breadcrumb', 'triphub' ),
			)
		)
	);
    ...
    $wp_customize->add_control(
        new triphub_Text_Control( 
            ...
            array(
                'type'            => 'text',
                'section'         => 'seo_settings',
                'label'           => __( 'Breadcrumb Home Text', 'triphub' ),
                'active_callback' => 'triphub_seo_breadcrumb_ac'
            )
        )
    );
    ...
    $wp_customize->add_control(
		new triphub_Radio_Image_Control(
			...
			array(
				'section'     => 'seo_settings',
				'label'       => __( 'Separator', 'triphub' ),
				'svg'         => true,
				'col'         => 'col-3',
				'choices'     => array(
                    'one' => array(
                        'label' => __( 'Type 1', 'triphub' ),
    ...
    $wp_customize->add_control(
		new triphub_Toggle_Control( 
			...
			array(
				'section'     => 'seo_settings',
				'label'	      => __( 'Show Last Updated Post Date', 'triphub' ),
                'description' => __( 'Enable to show last updated post date on listing as well as in single post.', 'triphub' ),
			)
		)
	);
}
add_action( 'customize_register', 'triphub_customize_register_seo_setting' );