...
if ( ! class_exists( 'triphub_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class triphub_WebFont_Loader {
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $url ) {
					...
					$triphub_path = download_url( $url );

					// Make sure there were no errors.
					if ( is_wp_error( $triphub_path ) ) {
						continue;
					}

					// Move temp file to final destination.
					$success = $this->get_filesystem()->move( $triphub_path, $font_path, true );
					if ( $success ) {
						$stored[ $url ] = $font_path;
						$change         = true;
					}
		...
		public function preload_local_fonts() {
			...
			update_site_option( 'triphub_local_font_files', $local_font );
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'triphub_get_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_base_url() {
			if ( ! $this->base_url ) {
				$this->base_url = apply_filters( 'triphub_get_local_fonts_base_url', content_url() );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'triphub_get_local_fonts_subfolder_name', 'fonts' );
			}
...
if ( ! function_exists( 'triphub_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function triphub_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new triphub_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'triphub_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function triphub_get_webfont_url( $url, $format = 'woff2' ) {
		$font = new triphub_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
...
if( ! function_exists( 'triphub_preload_local_fonts' ) ): 
/**
...
* @param string $format
*/
function triphub_preload_local_fonts( $url, $format = 'woff2' ) { 

	$fontcass = new triphub_WebFont_Loader( $url );
	...
	foreach ( $font_files as $font_family => $files ) {
		if ( is_array( $files ) ) {
			...
			update_site_option( 'triphub_local_font_files', $newFontArray );
		}
	}
	
	// Check if cache font files present or not
    $local_font_files = get_site_option( 'triphub_local_font_files', false );
	
    if ( is_array( $local_font_files ) && ! empty( $local_font_files ) ) {
        $font_format = apply_filters( 'travel_moster_pro_local_google_fonts_format', $format );
