( function( $, api ) {
	...
	api.controlConstructor['spacing'] = api.Control.extend( {
		ready: function() {
			var control = this;
			$( '.triphub-number-control', control.container ).on( 'change keyup',
				function() {
	...
	api.controlConstructor['triphub-spacing-slider'] = api.Control.extend( {
		ready: function() {
			var control = this;
	...
	api.controlConstructor['triphub-spacing'] = api.Control.extend( {
		ready: function() {
			var control = this,
				controlClass = '.customize-control-triphub-spacing',
				footerActions = jQuery( '#customize-footer-actions' );

			// Figure out which device icon to make active on load
			jQuery( controlClass + ' .triphub-spacing-control-section' ).each( function() {
...
jQuery( document ).ready( function($) {
	$( '.triphub-link-spacing' ).on( 'click', function(e) {
		e.preventDefault();
		...
		_this.parent( '.triphub-spacing-section' ).prevAll().slice(0,4).find( 'input' ).addClass( 'linked-values' ).attr( 'data-element', element );
	...
	$( '.triphub-unlink-spacing' ).on( 'click', function(e) {
		e.preventDefault();
		...
		_this.parent( '.triphub-spacing-section' ).prevAll().slice(0,4).find( 'input' ).removeClass( 'linked-values' ).attr( 'data-element', '' );
	...
	$( '.triphub-spacing-section' ).on( 'input', '.linked-values', function() {
		var _this = $( this ),
