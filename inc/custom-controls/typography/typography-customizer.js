		...
		ready: function() {
			...
			control.container.on( 'change', '.triphub-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the triphub_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'triphub_get_all_google_fonts_ajax',
								triphub_customize_nonce: triphub_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
